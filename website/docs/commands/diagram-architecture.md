...
**Trigger:** `"Diagram architecture"` or `/deepspec.diagram-architecture`
...
`ARCHITECTURE.md` is optional and generated on demand — it is not part of the core A-B-C set. Use `/deepspec.create-task` for planning; run this command when a visual architecture artifact helps review.
